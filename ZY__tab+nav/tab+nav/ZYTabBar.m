...
#import "ZY_NormalViewController.h"
...
#pragma mark-@interface ZYTabBarButton
...
-(void)createControllerBycontrollerArrayay:(NSArray *)controllerArrayay
{
    ...
    for (NSString *className in controllerArrayay) {
        ...
        JPNavigationController *nav = [[JPNavigationController alloc] initWithRootViewController:viewcontroller];
...
-(void)createTabBarView
{
    ...
    for(int i=0;i<num;i++)
    {
        ZYTabBarButton *button = [[ZYTabBarButton alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/num*i, 0, [UIScreen mainScreen].bounds.size.width/num,self.tabBarHeight)];
        button.tag = 1000+i;
